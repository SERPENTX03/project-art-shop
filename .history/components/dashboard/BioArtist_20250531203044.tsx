...
const BioEditor = ({ initialBio }: { initialBio?: string | null }) => {
  ...
  return (
    <div className="space-y-2 group relative w-fit border">
      <p className="text-gray-800 whitespace-pre-line">{bio}</p>
      <button
        onClick={() => setIsEditing(true)}
        className="absolute top-1/2 -translate-y-1/2 left-full text-sm bg-white p-1 rounded-full text-primary opacity-0 group-hover:opacity-100"
      >
        <Edit2 size={16} />
      </button>
    </div>
