...
export default function AdminSoldGallery({ galleries, pageSize = 6 }: Props) {
  ...
  // ตั้งค่าเริ่มต้นให้เป็นข้อมูลจาก props
  const [unpaidGalleries, setUnpaidGalleries] =
    useState<GalleryItem[]>(galleries);
  const [totalPage, setTotalPage] = useState(
    Math.ceil(galleries.length / pageSize)
  );
  ...
  const [pageItems, setPageItems] = useState<GalleryItem[]>(
    galleries.slice(0, pageSize) // ตั้งค่าเริ่มต้นให้แสดงหน้าแรก
  );
  ...
  const fetchPaidIds = useCallback(async () => {
    try {
      ...
      // กรองข้อมูลทันทีหลังจากได้รับ paidIds
      const filtered = galleries.filter((g) => !ids.includes(g.id));
      setUnpaidGalleries(filtered);
      setTotalPage(Math.ceil(filtered.length / pageSize));
      setPageItems(filtered.slice(0, pageSize));
    } catch (err) {
  ...
  useEffect(() => {
    // เราจะทำการอัพเดตแม้ว่าจะยังโหลดอยู่ เพื่อให้มีข้อมูลเริ่มต้น
    const filtered = galleries.filter((g) => !paidIds.includes(g.id));
    setUnpaidGalleries(filtered);
    setTotalPage(Math.ceil(filtered.length / pageSize));
  }, [paidIds, galleries, pageSize]);

  // อัพเดต pageItems เมื่อ unpaidGalleries หรือ page เปลี่ยน
  useEffect(() => {
    const items = calculatePageItems();
    console.log("PageItems updated:", items.length, "items");
    setPageItems(items);
  }, [unpaidGalleries, calculatePageItems, page]);
  ...
  useEffect(() => {
    console.log("Initial galleries:", galleries.length);
    fetchPaidIds();
  }, [fetchPaidIds]);
  ...
  const handlePayout = async (
    ...
    try {
      ...
      if (data.success) {
        ...
        setUnpaidGalleries((prev) => {
          const updated = prev.filter((g) => g.id !== galleryId);
          console.log("Updated unpaidGalleries:", updated.length);
          return updated;
        });

        // อัพเดต pageItems ด้วยเพื่อให้การแสดงผลอัพเดตทันที
        setPageItems((prev) => prev.filter((g) => g.id !== galleryId));

        // ตรวจสอบว่าหน้าปัจจุบันยังมีข้อมูลหรือไม่
        const currentPageIndex = (page - 1) * pageSize;
        if (unpaidGalleries.length - 1 <= currentPageIndex && page > 1) {
          setPage(page - 1);
        }
      }
    } catch (err) {
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {pageItems.map((gallery) => {
            ...
            const isItemLoading = loadingIds.includes(gallery.id);
            ...
            return (
              <Card key={gallery.id}>
                <CardContent className="p-4 space-y-2">
                  ...
                  <Button
                    disabled={isItemLoading || isPaid || !shop}
                    onClick={() =>
                      handlePayout(
                        gallery.id,
                        shop!.id,
