...
export default function AdminSoldGallery({ galleries, pageSize = 6 }: Props) {
  ...
  // 🔁 กรองเฉพาะ gallery ที่ยังไม่จ่าย
  ...
  useEffect(() => {
    // โหลดประวัติการโอนเพื่อเอาไว้กรอง
    const fetchPaidIds = async () => {
      try {
        ...
        const data = await res.json();
        console.log("Payout history data:", data);

        if (Array.isArray(data)) {
          const ids: string[] = data.map(
            (p: { galleryId: string }) => p.galleryId
          );
          console.log("Setting paid IDs:", ids);
          setPaidIds(ids);
        } else {
  ...
  useEffect(() => {
    ...
    console.log("Unpaid galleries:", unpaidGalleries.length);
    console.log("Current page:", page, "Total pages:", totalPage);
  }, [galleries, paidIds, unpaidGalleries, page, totalPage]);
