  ...
  if (eventType === "user.created") {
    ...
    try {
      ...
      const user = await prisma.user.create({
        data: {
          clerkId: evt.data.id,
          email: primaryEmail, // ไม่จำเป็นต้องใช้ || "" เนื่องจากตรวจสอบแล้วข้างต้น
          profileImage: evt.data.image_url || "", // ตั้งค่า default หากไม่มี
          firstName: evt.data.first_name || null, // ใช้ null แทน empty string
